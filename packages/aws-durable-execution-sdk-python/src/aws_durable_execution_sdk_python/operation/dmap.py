...
def _distributed_map_status_from_operation(
    ...
    The backend moved the distributed map run status off ``DistributedMapDetails``
    and onto the operation, so a terminal details block can omit ``Status``.
    ``_resolve_summary`` runs only on the success path, so this maps the
    operation's terminal status to the matching ``DistributedMapStatus``.
    """
